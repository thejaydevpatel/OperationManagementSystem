...
export default function SupplierReportPage() {
  ...
  const [supplier, setSupplier] = useState("");        // applied
  const [tempSupplier, setTempSupplier] = useState(""); // selecting
  ...
  const fetchReport = async (
    selectedSupplier: string = supplier,
  ...
  useEffect(() => {
    loadSuppliers();
    fetchReport("", today); // ✅ load today's report
  }, []);
  // ================= ACTIONS =================
  
  const search = () => {
    setSupplier(tempSupplier);
    fetchReport(tempSupplier, date);
  };
  ...
  const tableRows = useMemo(() => {
    ...
    if (supplier) {
      temp = temp.filter(
        (r: any) =>
          String(r?.supplier_id || "") === String(supplier)
      );
    }
