...
//   return (
//  <div className="p-6 min-h-screen mt-10 bg-gray-50 dark:bg-black">
...
//                               size="icon"
//                               variant="outline"
//                               className="p-2 cursor-pointer bg-blue-200 dark:bg-blue-200 rounded-full"
...
import DriverModal from "@/app/components/allocate/DriverModal";
// import GuideModal from "./GuideModal";
import GuideModal from "@/app/components/allocate/GuideModal";
export default function AllocatePage() {
  const { bookingId } = useParams();
  ...
  useEffect(() => {
    ...
    fetch("/api/duties/drivers").then(res => res.json()).then(setDrivers);
    fetch("/api/duties/vehicles").then(res => res.json()).then(setVehicles);
    fetch("/api/duties/suppliers").then(res => res.json()).then(setSuppliers);
    fetch("/api/duties/tour-guides").then(res => res.json()).then(setTourGuides);
    fetch("/api/duties/languages").then(res => res.json()).then(setLanguages);
  ...
  const handleOpenGuideModal = (jobId: number) => {
    setSelectedJobId(jobId);
    setIsGuideModalOpen(true);
  };
