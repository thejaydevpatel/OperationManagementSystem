...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM operation_logs_lookup_operation_logs_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE operation_logs_lookup_operation_logs_table 
    SET ${setClause} 
    ...
    console.error("Error updating operation_logs_lookup_operation_logs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting operation_logs_lookup_operation_logs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status operation_logs_lookup_operation_logs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
