...
import { UsersTableEntity } from "./interface/users-table";
...
export async function GET(req: NextRequest) {
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM users_lookup_users_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM users_lookup_users_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching users_lookup_users_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<UsersTableEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO users_lookup_users_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting users_lookup_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
