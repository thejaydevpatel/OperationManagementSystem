...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM users_lookup_users_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE users_lookup_users_table 
    SET ${setClause} 
    ...
    console.error("Error updating users_lookup_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting users_lookup_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status users_lookup_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
