...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM placard_details_lookup_placard_details_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE placard_details_lookup_placard_details_table 
    SET ${setClause} 
    ...
    console.error("Error updating placard_details_lookup_placard_details_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting placard_details_lookup_placard_details_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status placard_details_lookup_placard_details_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
