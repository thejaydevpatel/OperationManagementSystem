...
import { NotificationEntity } from "./interface/notification";
...
export async function GET(req: NextRequest) {
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM lookup_notification WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM lookup_notification WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching lookup_notification:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<NotificationEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO lookup_notification (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting lookup_notification:", err);
    return ApiResponse.failed("", startTime);
  }
}
