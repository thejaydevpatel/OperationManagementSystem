...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM shared_group_jobs_lookup_shared_group_jobs_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE shared_group_jobs_lookup_shared_group_jobs_table 
    SET ${setClause} 
    ...
    console.error("Error updating shared_group_jobs_lookup_shared_group_jobs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting shared_group_jobs_lookup_shared_group_jobs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status shared_group_jobs_lookup_shared_group_jobs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
