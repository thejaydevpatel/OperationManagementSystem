...
import { SharedGroupJobsTableEntity } from "./interface/shared-group-jobs-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","group_id","job_id","pax_assigned","pickup_order_no","drop_order_no","status_id"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM shared_group_jobs_lookup_shared_group_jobs_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM shared_group_jobs_lookup_shared_group_jobs_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "shared_group_jobs_lookup_shared_group_jobs_table"
            // );
    ...
    console.error("Error fetching shared_group_jobs_lookup_shared_group_jobs_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<SharedGroupJobsTableEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO shared_group_jobs_lookup_shared_group_jobs_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting shared_group_jobs_lookup_shared_group_jobs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
