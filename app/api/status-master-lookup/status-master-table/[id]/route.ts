...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM status_master_lookup_status_master_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE status_master_lookup_status_master_table 
    SET ${setClause} 
    ...
    console.error("Error updating status_master_lookup_status_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting status_master_lookup_status_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status status_master_lookup_status_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
