...
import { AdminUsersEntity } from "./interface/admin-users";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","name","email","password","role"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM master_lookup_admin_users WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM master_lookup_admin_users WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching master_lookup_admin_users:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<AdminUsersEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO master_lookup_admin_users (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting master_lookup_admin_users:", err);
    return ApiResponse.failed("", startTime);
  }
}
