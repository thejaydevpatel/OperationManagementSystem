...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM master_lookup_modules WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    console.error("Error updating master_lookup_modules:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting master_lookup_modules:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status master_lookup_modules:", err);
    return ApiResponse.failed("", startTime);
  }
}
