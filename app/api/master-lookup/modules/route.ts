...
import { ModulesEntity } from "./interface/modules";
...
export async function GET(req: NextRequest) {
 ...
 const allowedSortFields = [
  ...
  "deleted_by","tenant_id","id","name","icon","order_index"
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM master_lookup_modules WHERE is_deleted=false  ORDER BY order_index ASC LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM master_lookup_modules WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching master_lookup_modules:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<ModulesEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO master_lookup_modules (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting master_lookup_modules:", err);
    return ApiResponse.failed("", startTime);
  }
}
