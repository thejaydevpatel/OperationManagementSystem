...
import { SupplierMasterTableEntity } from "./interface/supplier-master-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","supplier_type","name","service_type","contact_person","phone","email","address","status_id"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM supplier_master_lookup_supplier_master_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM supplier_master_lookup_supplier_master_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "supplier_master_lookup_supplier_master_table"
            // );
    ...
    console.error("Error fetching supplier_master_lookup_supplier_master_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<SupplierMasterTableEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO supplier_master_lookup_supplier_master_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting supplier_master_lookup_supplier_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
