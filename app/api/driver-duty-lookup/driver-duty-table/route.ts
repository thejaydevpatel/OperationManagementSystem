...
import { DriverDutyTableEntity } from "./interface/driver-duty-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","driver_id","duty_start_time","duty_end_time","total_hours","status_id","notes"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM driver_duty_lookup_driver_duty_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM driver_duty_lookup_driver_duty_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "driver_duty_lookup_driver_duty_table"
            // );
    ...
    console.error("Error fetching driver_duty_lookup_driver_duty_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<DriverDutyTableEntity> = await req.json();
    ...
    const query = `INSERT INTO driver_duty_lookup_driver_duty_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting driver_duty_lookup_driver_duty_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
