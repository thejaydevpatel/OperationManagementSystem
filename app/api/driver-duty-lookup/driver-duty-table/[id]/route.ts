...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM driver_duty_lookup_driver_duty_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE driver_duty_lookup_driver_duty_table 
    SET ${setClause} 
    ...
    console.error("Error updating driver_duty_lookup_driver_duty_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting driver_duty_lookup_driver_duty_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status driver_duty_lookup_driver_duty_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
