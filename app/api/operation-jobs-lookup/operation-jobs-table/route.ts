...
import { OperationJobsTableEntity } from "./interface/operation-jobs-table";
...
export async function GET(req: NextRequest) {
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM operation_jobs_lookup_operation_jobs_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM operation_jobs_lookup_operation_jobs_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "operation_jobs_lookup_operation_jobs_table"
            // );
    ...
    console.error("Error fetching operation_jobs_lookup_operation_jobs_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<OperationJobsTableEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO operation_jobs_lookup_operation_jobs_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting operation_jobs_lookup_operation_jobs_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
