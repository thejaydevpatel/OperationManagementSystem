...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM d_lookup_d_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    console.error("Error updating d_lookup_d_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting d_lookup_d_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status d_lookup_d_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
