...
import { DTableEntity } from "./interface/d-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","vehicle_id"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM d_lookup_d_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM d_lookup_d_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching d_lookup_d_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<DTableEntity> = await req.json();
    ...
    const query = `INSERT INTO d_lookup_d_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting d_lookup_d_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
