...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM agents_lookup_agents_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE agents_lookup_agents_table 
    SET ${setClause} 
    ...
    console.error("Error updating agents_lookup_agents_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting agents_lookup_agents_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status agents_lookup_agents_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
