...
import { SharedGroupRoutePointsTableEntity } from "./interface/shared-group-route-points-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","group_id","order_no","locarion_id","stop_type","eta","actual_arrival","actual_departure","status_id","notes"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM shared_group_route_points_lookup_shared_group_route_points_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM shared_group_route_points_lookup_shared_group_route_points_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "shared_group_route_points_lookup_shared_group_route_points_table"
            // );
    ...
    console.error("Error fetching shared_group_route_points_lookup_shared_group_route_points_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<SharedGroupRoutePointsTableEntity> = await req.json();
    ...
    const query = `INSERT INTO shared_group_route_points_lookup_shared_group_route_points_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting shared_group_route_points_lookup_shared_group_route_points_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
