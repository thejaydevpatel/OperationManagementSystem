...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM guide_price_lookup_guide_price_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE guide_price_lookup_guide_price_table 
    SET ${setClause} 
    ...
    console.error("Error updating guide_price_lookup_guide_price_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting guide_price_lookup_guide_price_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status guide_price_lookup_guide_price_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
