...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","supplier_type","supplier_id","name","phone","license_number","driver_status_id","last_known_location_id","last_duty_end_time","max_daily_hours","notes"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM drivers_lookup_drivers_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM drivers_lookup_drivers_table WHERE is_deleted=false`,   []);
    const totalRecords =
...
export async function POST(req: NextRequest) {
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO drivers_lookup_drivers_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting drivers_lookup_drivers_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
