...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM driver_availability_lookup_driver_availability_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE driver_availability_lookup_driver_availability_table 
    SET ${setClause} 
    ...
    console.error("Error updating driver_availability_lookup_driver_availability_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting driver_availability_lookup_driver_availability_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
//   } catch (err) {
//     console.error("Error updating status driver_availability_lookup_driver_availability_table:", err);
//     return ApiResponse.failed("", startTime);
//   }
// }
...
export async function PATCH(
  ...
  try {
    ...
    // ⭐ whitelist (VERY IMPORTANT SECURITY)
    const allowedFields = [
      "is_active",
      "is_available",
    ];
    ...
    if (!allowedFields.includes(field)) {
      return ApiResponse.failed("Invalid field", startTime);
    }
    ...
    return ApiResponse.updated(id, startTime);

  } catch (err) {
    console.error(
      "Error updating toggle driver_availability_lookup_driver_availability_table:",
      err
    );
    return ApiResponse.failed("", startTime);
  }
