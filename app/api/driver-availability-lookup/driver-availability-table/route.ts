...
import { DriverAvailabilityTableEntity } from "./interface/driver-availability-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","driver_id","is_available","current_location_id","current_job_id","next_free_time"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM driver_availability_lookup_driver_availability_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM driver_availability_lookup_driver_availability_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "driver_availability_lookup_driver_availability_table"
            // );
    ...
    console.error("Error fetching driver_availability_lookup_driver_availability_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<DriverAvailabilityTableEntity> = await req.json();
    ...
    const query = `INSERT INTO driver_availability_lookup_driver_availability_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting driver_availability_lookup_driver_availability_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
