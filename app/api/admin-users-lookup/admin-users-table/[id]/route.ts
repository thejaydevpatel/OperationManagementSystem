...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM admin_users_lookup_admin_users_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE admin_users_lookup_admin_users_table 
    SET ${setClause} 
    ...
    console.error("Error updating admin_users_lookup_admin_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting admin_users_lookup_admin_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status admin_users_lookup_admin_users_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
