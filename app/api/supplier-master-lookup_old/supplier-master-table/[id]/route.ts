...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM supplier_master_lookup_supplier_master_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE supplier_master_lookup_supplier_master_table 
    SET ${setClause} 
    ...
    console.error("Error updating supplier_master_lookup_supplier_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting supplier_master_lookup_supplier_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status supplier_master_lookup_supplier_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
