...
import { CTableEntity } from "./interface/c-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","name","status_id"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM c_lookup_c_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM c_lookup_c_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching c_lookup_c_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<CTableEntity> = await req.json();
    ...
    const query = `INSERT INTO c_lookup_c_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting c_lookup_c_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
