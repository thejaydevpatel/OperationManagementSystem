...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM job_route_points_lookup_job_route_points_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE job_route_points_lookup_job_route_points_table 
    SET ${setClause} 
    ...
    console.error("Error updating job_route_points_lookup_job_route_points_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting job_route_points_lookup_job_route_points_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status job_route_points_lookup_job_route_points_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
