...
import { FTableEntity } from "./interface/f-table";
...
export async function GET(req: NextRequest) {
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM f_lookup_f_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM f_lookup_f_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching f_lookup_f_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<FTableEntity> = await req.json();
    ...
    const query = `INSERT INTO f_lookup_f_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting f_lookup_f_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
