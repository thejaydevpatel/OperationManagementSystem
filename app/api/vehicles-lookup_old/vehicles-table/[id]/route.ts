...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM vehicles_lookup_vehicles_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE vehicles_lookup_vehicles_table 
    SET ${setClause} 
    ...
    console.error("Error updating vehicles_lookup_vehicles_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting vehicles_lookup_vehicles_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status vehicles_lookup_vehicles_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
