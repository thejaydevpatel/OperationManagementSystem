...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM shared_groupings_lookup_shared_groupings_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE shared_groupings_lookup_shared_groupings_table 
    SET ${setClause} 
    ...
    console.error("Error updating shared_groupings_lookup_shared_groupings_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting shared_groupings_lookup_shared_groupings_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status shared_groupings_lookup_shared_groupings_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
