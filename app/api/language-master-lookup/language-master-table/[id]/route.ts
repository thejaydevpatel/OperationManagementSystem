...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM language_master_lookup_language_master_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE language_master_lookup_language_master_table 
    SET ${setClause} 
    ...
    console.error("Error updating language_master_lookup_language_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting language_master_lookup_language_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status language_master_lookup_language_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
