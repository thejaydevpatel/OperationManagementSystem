...
import { LanguageMasterTableEntity } from "./interface/language-master-table";
...
export async function GET(req: NextRequest) {
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM language_master_lookup_language_master_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM language_master_lookup_language_master_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "language_master_lookup_language_master_table"
            // );
    ...
    console.error("Error fetching language_master_lookup_language_master_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<LanguageMasterTableEntity> = await req.json();
    ...
    const query = `INSERT INTO language_master_lookup_language_master_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting language_master_lookup_language_master_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
