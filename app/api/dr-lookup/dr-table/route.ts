...
import { DrTableEntity } from "./interface/dr-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","job_id"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM dr_lookup_dr_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM dr_lookup_dr_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    console.error("Error fetching dr_lookup_dr_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<DrTableEntity> = await req.json();
    ...
    const query = `INSERT INTO dr_lookup_dr_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting dr_lookup_dr_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
