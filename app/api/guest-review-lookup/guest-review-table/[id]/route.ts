...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM guest_review_lookup_guest_review_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE guest_review_lookup_guest_review_table 
    SET ${setClause} 
    ...
    console.error("Error updating guest_review_lookup_guest_review_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting guest_review_lookup_guest_review_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
//   } catch (err) {
//     console.error("Error updating status guest_review_lookup_guest_review_table:", err);
//     return ApiResponse.failed("", startTime);
//   }
// }
...
export async function PATCH(
  ...
  try {
    ...
    console.error(
      "Error updating status guest_review_lookup_guest_review_table:",
      err
    );
    return ApiResponse.failed("", startTime);
  }
