...
import { GuestReviewTableEntity } from "./interface/guest-review-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","job_id","review_target_type","review_target_id","rating","review_text","guest_name","guest_email","is_anonymous","is_verified"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM guest_review_lookup_guest_review_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM guest_review_lookup_guest_review_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "guest_review_lookup_guest_review_table"
            // );
    ...
    console.error("Error fetching guest_review_lookup_guest_review_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<GuestReviewTableEntity> = await req.json();
 ...
 Object.keys(body).forEach((key) => {
    ...
    const query = `INSERT INTO guest_review_lookup_guest_review_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting guest_review_lookup_guest_review_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
