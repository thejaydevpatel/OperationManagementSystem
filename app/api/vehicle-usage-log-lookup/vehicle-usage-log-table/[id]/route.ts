...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM vehicle_usage_log_lookup_vehicle_usage_log_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE vehicle_usage_log_lookup_vehicle_usage_log_table 
    SET ${setClause} 
    ...
    console.error("Error updating vehicle_usage_log_lookup_vehicle_usage_log_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting vehicle_usage_log_lookup_vehicle_usage_log_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status vehicle_usage_log_lookup_vehicle_usage_log_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
