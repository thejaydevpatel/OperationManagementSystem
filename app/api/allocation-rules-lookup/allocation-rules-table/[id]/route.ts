...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM allocation_rules_lookup_allocation_rules_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    const query = `
    UPDATE allocation_rules_lookup_allocation_rules_table 
    SET ${setClause} 
    ...
    console.error("Error updating allocation_rules_lookup_allocation_rules_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting allocation_rules_lookup_allocation_rules_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status allocation_rules_lookup_allocation_rules_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
