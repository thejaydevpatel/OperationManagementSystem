...
import { AllocationRulesTableEntity } from "./interface/allocation-rules-table";
...
export async function GET(req: NextRequest) {
  ...
  const { searchParams } = new URL(req.url);
    ...
    const allowedSortFields = ["is_used","is_deleted","is_active","created_at","updated_at","created_by","updated_by","host_ip","url","deleted_at","deleted_by","tenant_id","id","rule_key","rule_name","rule_type","rule_value","scope_type","scope_value","priority","effective_from","effective_to"]; // adjust to your table columns
  ...
  const client = getDbConnection(dbName);
    const result = await client.query(`SELECT * FROM allocation_rules_lookup_allocation_rules_table WHERE is_deleted=false  ORDER BY ${sortBy} ${order} LIMIT $1 OFFSET $2`,
      [pageSize, offset]);

      
    const countResult = await client.query(`SELECT COUNT(*)::int AS total FROM allocation_rules_lookup_allocation_rules_table WHERE is_deleted=false`,   []);
    const totalRecords =
    ...
    const totalPages =
      ...
      if (result?.rows.length > 0) {
            ...
            //   dbName,
            //   "allocation_rules_lookup_allocation_rules_table"
            // );
    ...
    console.error("Error fetching allocation_rules_lookup_allocation_rules_table:", err);
...
export async function POST(req: NextRequest) {
  ...
  const body: Partial<AllocationRulesTableEntity> = await req.json();
...
Object.keys(body).forEach((key) => {
  if (body[key as keyof AllocationRulesTableEntity] === "") {
    body[key as keyof AllocationRulesTableEntity] = null as any;
  }
});
    ...
    const query = `INSERT INTO allocation_rules_lookup_allocation_rules_table (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`;
    const result = await client.query(query, values);
    return ApiResponse.created(result.rows[0].id, startTime);
  } catch (err) {
    console.error("Error inserting allocation_rules_lookup_allocation_rules_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
