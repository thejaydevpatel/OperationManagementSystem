...
export async function GET(
  ...
  try {
    ...
    const result = await client.query(
      `SELECT * FROM driver_allocation_lookup_driver_allocation_table WHERE id = $1`,
      [Number(id)]
    );
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const referer = req.headers.get("referer");
    ...
    console.error("Error updating driver_allocation_lookup_driver_allocation_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function DELETE(
  ...
  const referenced = await PrepareAndDispatchValidation(
    ...
    console.error("Error deleting driver_allocation_lookup_driver_allocation_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("Error updating status driver_allocation_lookup_driver_allocation_table:", err);
    return ApiResponse.failed("", startTime);
  }
}
