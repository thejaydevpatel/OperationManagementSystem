...
interface drivers_tablePaginationTableProps {
  rows: DriversTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<DriversTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof DriversTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof DriversTableEntity>>;
...
const List: React.FC<drivers_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [driver_status_id, setDriver_status_id] = React.useState<any[]>([]);


const [last_known_location_id, setLast_known_location_id] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchDriver_status_id = async () => {
    const res = await fetch("/api/status-master-lookup/status-master-table?pageSize=9999");
    const data = await res.json();
    setDriver_status_id(data.data || []);
  };

  fetchDriver_status_id();
}, []);


React.useEffect(() => {
  const fetchLast_known_location_id = async () => {
    const res = await fetch("/api/location-master-lookup/location-master-table?pageSize=9999");
    const data = await res.json();
    setLast_known_location_id(data.data || []);
  };

  fetchLast_known_location_id();
}, []);
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Last Known Location":
      last_known_location_id.find(o => Number(o.id) === Number(row.last_known_location_id))?.name || "-",
    "Last Duty End Time": row.last_duty_end_time
      ? new Date(row.last_duty_end_time).toLocaleString("en-GB")
      : "-",
    "Max Daily Hours": row.max_daily_hours || "-",
    "Notes": row.notes || "-",
    "Status": row.is_active ? "Active" : "Suspended",
  }));

  exportToExcel(exportData, "Drivers_Table");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Drivers Table yet`}
        description="Create your first record to get started."
        buttonText={`New Drivers Table`}
       buttonHref="/dashboard/drivers-lookup/drivers-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/drivers-lookup/drivers-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/drivers-lookup/drivers-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
