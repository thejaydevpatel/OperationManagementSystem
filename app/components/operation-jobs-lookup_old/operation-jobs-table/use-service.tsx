...
import { getOperationJobsTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  z.object({
  external_booking_id: z.string().min(1, "external_booking_id is required").max(80),
  booking_type: z.string().min(1, "booking_type is required"),
  service_type: z.string().min(1, "service_type is required"),
  pax: z.number().int(),
  vehicle_type_required: z.number().int().optional(),
  pickup_location_id: z.number().int(),
  dropoff_location_id: z.number().int().optional(),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof OperationJobsTableEntity>(
      (searchParams.get("sort") as keyof OperationJobsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<OperationJobsTableEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [vehicle_type_required, setVehicle_type_required] = useState<any[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  vehicle_type_required: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/vehicle-types-lookup/vehicle-types-table?pageSize=9999`,
  ...
  // const customResolver: Resolver<OperationJobsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<OperationJobsTableEntity>({
 defaultValues: {
  external_booking_id: "",
  booking_type: "",
  service_type: "",
  ...
  const onSubmit: SubmitHandler<OperationJobsTableEntity> = async (values) => {
    // ✅ Validation
  
  if (values.external_booking_id === undefined || values.external_booking_id === null) {
    toast.error("external_booking_id is required");
    return;
  }

  if (values.booking_type === undefined || values.booking_type === null) {
    toast.error("booking_type is required");
    return;
  }
  ...
  if (values.pax === undefined || values.pax === null) {
    toast.error("pax is required");
    return;
  }

  if (values.pickup_location_id === undefined || values.pickup_location_id === null) {
    toast.error("pickup_location_id is required");
    return;
  }

  if (values.scheduled_start_time === undefined || values.scheduled_start_time === null) {
    toast.error("scheduled_start_time is required");
    return;
  }

  if (values.job_status_id === undefined || values.job_status_id === null) {
    toast.error("job_status_id is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: OperationJobsTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: OperationJobsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as OperationJobsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof OperationJobsTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as OperationJobsTableEntity);
    setEditId(0);
  };
...
if (key === "pickup_location_id") {
      ...
      if (endPoint) {
      const result = await handleRefreshAPIs(endPoint);
      setPickup_location_id(result.data);
    }
    }

if (key === "dropoff_location_id") {
      const endPoint =dropdownEndpoints[key]; 
      if (endPoint) {
      const result = await handleRefreshAPIs(endPoint);
      setDropoff_location_id(result.data);
    }
    }
