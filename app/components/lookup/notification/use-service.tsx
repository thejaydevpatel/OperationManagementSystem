...
import { getNotificationEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof NotificationEntity>(
      (searchParams.get("sort") as keyof NotificationEntity) || "id"
    );
  ...
  const [list, setList] = useState<NotificationEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getNotificationEntityApi(module);

 
  // const customResolver: Resolver<NotificationEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<NotificationEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: NotificationEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as NotificationEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof NotificationEntity)[]).forEach((key) => {
