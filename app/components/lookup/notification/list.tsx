...
interface notificationPaginationTableProps {
  ...
  setList: React.Dispatch<React.SetStateAction<NotificationEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof NotificationEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof NotificationEntity>>;
...
const List: React.FC<notificationPaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Notification yet`}
        description="Create your first record to get started."
        buttonText={`New Notification`}
       buttonHref="/dashboard/lookup/notification/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/lookup/notification/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/lookup/notification/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
