...
import { GuideLanguagePriceEntity } from "@/app/api/lookup/guide-language-price/interface/guide-language-price";

interface guide_language_pricePaginationTableProps {
  rows: GuideLanguagePriceEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<GuideLanguagePriceEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof GuideLanguagePriceEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof GuideLanguagePriceEntity>>;
...
const List: React.FC<guide_language_pricePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [language_id, setLanguage_id] = React.useState<any[]>([]);
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    "Sr No.": index + 1,
    "Guide Name":
      guide_id.find((g) => Number(g.id) === Number(row.guide_id))?.name || "-",
    "Language Name":
      language_id.find((l) => Number(l.id) === Number(row.language_id))?.name || "-",
    "Price": row.price ?? "-",
    "Status": row.is_active ? "Active" : "Suspended",
  }));

  exportToExcel(exportData, "Guide_Language_Price");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Guide Language Price yet`}
        description="Create your first record to get started."
        buttonText={`New Guide Language Price`}
       buttonHref="/dashboard/lookup/guide-language-price/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/lookup/guide-language-price/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/lookup/guide-language-price/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
