...
import { getGuideLanguagePriceEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof GuideLanguagePriceEntity>(
      (searchParams.get("sort") as keyof GuideLanguagePriceEntity) || "id"
    );
  ...
  const [list, setList] = useState<GuideLanguagePriceEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [guide_id, setGuide_id] = useState<any[]>([]);
 const [language_id, setLanguage_id] = useState<any[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  language_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/language-master-lookup/language-master-table?pageSize=9999`,
};

  const api = getGuideLanguagePriceEntityApi(module);

 
  // const customResolver: Resolver<GuideLanguagePriceEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<GuideLanguagePriceEntity>({
 defaultValues: {
  price: 0,
},  
  ...
  const onSubmit: SubmitHandler<GuideLanguagePriceEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: GuideLanguagePriceEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: GuideLanguagePriceEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as GuideLanguagePriceEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof GuideLanguagePriceEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as GuideLanguagePriceEntity);
    setEditId(0);
  };
