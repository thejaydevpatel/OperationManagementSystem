...
import { StatusMasterTableEntity } from "@/app/api/status-master-lookup/status-master-table/interface/status-master-table"
   

import { getStatusMasterTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  z.object({
  status_master_id: z.number().int(),
  name: z.string().min(1, "name is required").max(100),
  status_code: z.string().min(1, "status_code is required").max(50),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof StatusMasterTableEntity>(
      (searchParams.get("sort") as keyof StatusMasterTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<StatusMasterTableEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [status_master_id, setStatus_master_id] = useState<any[]>([]);


const dropdownEndpoints: Record<string, string | null> = {
  status_master_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/status-category-lookup/status-category-table?pageSize=9999`,
};

  const api = getStatusMasterTableEntityApi(module);

 
  // const customResolver: Resolver<StatusMasterTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<StatusMasterTableEntity>({
 defaultValues: {
  name: "",
  status_code: "",
  sequence_order: 0,
  color_code: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"name":"","status_code":"","sequence_order":0,"color_code":""}
  });
  ...
  const onSubmit: SubmitHandler<StatusMasterTableEntity> = async (values) => {
    // ✅ Validation
  
  if (values.status_master_id === undefined || values.status_master_id === null) {
    toast.error("status_master_id is required");
    return;
  }
  ...
  if (values.status_code === undefined || values.status_code === null) {
    toast.error("status_code is required");
    return;
  }

  if (values.sequence_order === undefined || values.sequence_order === null) {
    toast.error("sequence_order is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: StatusMasterTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: StatusMasterTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as StatusMasterTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof StatusMasterTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as StatusMasterTableEntity);
    setEditId(0);
  };
  ...
  return {
    state: { register, list, editId, modal, setList,btnLoading, pageLoading, page,
      ...
      setDropDownloading,status_master_id, },
