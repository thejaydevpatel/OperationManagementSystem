...
interface status_master_tablePaginationTableProps {
  rows: StatusMasterTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<StatusMasterTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof StatusMasterTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof StatusMasterTableEntity>>;
...
const List: React.FC<status_master_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [status_master_id, setStatus_master_id] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchStatus_master_id = async () => {
    const res = await fetch("/api/status-category-lookup/status-category-table");
    const data = await res.json();
    setStatus_master_id(data.data || []);
  };

  fetchStatus_master_id();
}, []);
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Status Master Table yet`}
        description="Create your first record to get started."
        buttonText={`New Status Master Table`}
       buttonHref="/dashboard/status-master-lookup/status-master-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/status-master-lookup/status-master-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/status-master-lookup/status-master-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
