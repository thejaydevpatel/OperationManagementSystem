...
interface agents_tablePaginationTableProps {
  rows: AgentsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<AgentsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof AgentsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof AgentsTableEntity>>;
...
const List: React.FC<agents_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
 ...
 if (filteredRows.length === 1 && customPage > 0) {
  ...
  exportToExcel(exportData, "Agents_Table", "Agents");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Agents Table yet`}
        description="Create your first record to get started."
        buttonText={`New Agents Table`}
       buttonHref="/dashboard/agents-lookup/agents-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/agents-lookup/agents-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/agents-lookup/agents-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
