...
import { AgentsTableEntity } from "@/app/api/agents-lookup/agents-table/interface/agents-table"
   

import { getAgentsTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof AgentsTableEntity>(
      (searchParams.get("sort") as keyof AgentsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<AgentsTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getAgentsTableEntityApi(module);

 
  // const customResolver: Resolver<AgentsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<AgentsTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: AgentsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as AgentsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof AgentsTableEntity)[]).forEach((key) => {
