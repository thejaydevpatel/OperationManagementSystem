...
import { VehicleUsageLogTableEntity } from "@/app/api/vehicle-usage-log-lookup/vehicle-usage-log-table/interface/vehicle-usage-log-table";

interface vehicle_usage_log_tablePaginationTableProps {
  rows: VehicleUsageLogTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<VehicleUsageLogTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof VehicleUsageLogTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof VehicleUsageLogTableEntity>>;
...
const List: React.FC<vehicle_usage_log_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  ...
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Vehicle": vehicle_id.find(
      (o) => Number(o.id) === Number(row.vehicle_id)
    )?.vehicle_type_id || "-",

    "Job": job_id.find(
      (o) => Number(o.id) === Number(row.job_id)
    )?.external_booking_id || "-",

    "Driver": driver_id.find(
      (o) => Number(o.id) === Number(row.driver_id)
    )?.name || "-",
    ...
    "Distance Covered": row.distance_covered || "-",
  ...
  exportToExcel(exportData, "Vehicle_Usage_Log");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Vehicle Usage Log Table yet`}
        description="Create your first record to get started."
        buttonText={`New Vehicle Usage Log Table`}
       buttonHref="/dashboard/vehicle-usage-log-lookup/vehicle-usage-log-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/vehicle-usage-log-lookup/vehicle-usage-log-table/new"
                >
                  <Button className="min-w-[150px]">
                    <Plus className="mr-2 h-4 w-4" />
                    Add New Vehicle Usage Log
                  </Button>
                </Link>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/vehicle-usage-log-lookup/vehicle-usage-log-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
