...
import { getVehicleUsageLogTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  ...
  distance_covered: z.string().optional(),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof VehicleUsageLogTableEntity>(
      (searchParams.get("sort") as keyof VehicleUsageLogTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<VehicleUsageLogTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getVehicleUsageLogTableEntityApi(module);

 
  // const customResolver: Resolver<VehicleUsageLogTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<VehicleUsageLogTableEntity>({
 defaultValues: {
  start_time: "",
  end_time: "",
  distance_covered: 0,
  notes: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"distance_covered":0}
  });
  ...
  const onSubmit: SubmitHandler<VehicleUsageLogTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: VehicleUsageLogTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: VehicleUsageLogTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as VehicleUsageLogTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof VehicleUsageLogTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as VehicleUsageLogTableEntity);
    setEditId(0);
  };
