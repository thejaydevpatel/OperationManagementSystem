...
import { getSharedGroupingsTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof SharedGroupingsTableEntity>(
      (searchParams.get("sort") as keyof SharedGroupingsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<SharedGroupingsTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getSharedGroupingsTableEntityApi(module);

 
  // const customResolver: Resolver<SharedGroupingsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<SharedGroupingsTableEntity>({
 defaultValues: {
  start_time: "",
  end_time: "",
  max_capacity: 0,
  current_pax: 0,
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"max_capacity":0,"current_pax":0}
  });
  ...
  const onSubmit: SubmitHandler<SharedGroupingsTableEntity> = async (values) => {
    // ✅ Validation
  ...
  if (values.start_time === undefined || values.start_time === null) {
    toast.error("start_time is required");
    return;
  }

  if (values.max_capacity === undefined || values.max_capacity === null) {
    toast.error("max_capacity is required");
    return;
  }

  if (values.current_pax === undefined || values.current_pax === null) {
    toast.error("current_pax is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: SharedGroupingsTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: SharedGroupingsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as SharedGroupingsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof SharedGroupingsTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as SharedGroupingsTableEntity);
    setEditId(0);
  };
