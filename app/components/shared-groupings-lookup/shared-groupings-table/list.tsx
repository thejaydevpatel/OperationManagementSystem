...
import { SharedGroupingsTableEntity } from "@/app/api/shared-groupings-lookup/shared-groupings-table/interface/shared-groupings-table";

interface shared_groupings_tablePaginationTableProps {
  rows: SharedGroupingsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<SharedGroupingsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof SharedGroupingsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof SharedGroupingsTableEntity>>;
...
const List: React.FC<shared_groupings_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Vehicle":
      vehicle_id.find(o => Number(o.id) === Number(row.vehicle_id))
        ?.registration_number || "-",

    "Driver":
      driver_id.find(o => Number(o.id) === Number(row.driver_id))
        ?.name || "-",
    ...
    "Current Pax": row.current_pax || "-",

    "Status": row.is_active ? "Active" : "Suspended",
  }));

  exportToExcel(exportData, "Shared_Groupings");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Shared Groupings Table yet`}
        description="Create your first record to get started."
        buttonText={`New Shared Groupings Table`}
       buttonHref="/dashboard/shared-groupings-lookup/shared-groupings-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/shared-groupings-lookup/shared-groupings-table/new"
                >
                  <Button className="min-w-[150px]">
                    <Plus className="mr-2 h-4 w-4" />
                    Add New Shared Grouping
                  </Button>
                </Link>
...
<TableHead>Current Pax</TableHead>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/shared-groupings-lookup/shared-groupings-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
