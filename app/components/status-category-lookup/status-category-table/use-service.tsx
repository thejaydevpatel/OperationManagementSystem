...
import { getStatusCategoryTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof StatusCategoryTableEntity>(
      (searchParams.get("sort") as keyof StatusCategoryTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<StatusCategoryTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getStatusCategoryTableEntityApi(module);

 
  // const customResolver: Resolver<StatusCategoryTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<StatusCategoryTableEntity>({
 defaultValues: {
  name: "",
  description: "",
},  
  ...
  const onSubmit: SubmitHandler<StatusCategoryTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: StatusCategoryTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: StatusCategoryTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as StatusCategoryTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof StatusCategoryTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as StatusCategoryTableEntity);
    setEditId(0);
  };
