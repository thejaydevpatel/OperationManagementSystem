...
interface status_category_tablePaginationTableProps {
  rows: StatusCategoryTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<StatusCategoryTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof StatusCategoryTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof StatusCategoryTableEntity>>;
...
const List: React.FC<status_category_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Status Category Table yet`}
        description="Create your first record to get started."
        buttonText={`New Status Category Table`}
       buttonHref="/dashboard/status-category-lookup/status-category-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/status-category-lookup/status-category-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/status-category-lookup/status-category-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
