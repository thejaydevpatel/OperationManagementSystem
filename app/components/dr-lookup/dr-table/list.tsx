...
import { DrTableEntity } from "@/app/api/dr-lookup/dr-table/interface/dr-table";

interface dr_tablePaginationTableProps {
  rows: DrTableEntity[];
  ...
  setList: React.Dispatch<React.SetStateAction<DrTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof DrTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof DrTableEntity>>;
...
const List: React.FC<dr_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Dr Table yet`}
        description="Create your first record to get started."
        buttonText={`New Dr Table`}
       buttonHref="/dashboard/dr-lookup/dr-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/dr-lookup/dr-table/new"
                >
    ...
    {job_id.find(o => Number(o.id) === Number(row.job_id))?.name || "-"}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/dr-lookup/dr-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
