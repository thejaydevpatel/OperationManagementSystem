...
import { getDrTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof DrTableEntity>(
      (searchParams.get("sort") as keyof DrTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<DrTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getDrTableEntityApi(module);

 
  // const customResolver: Resolver<DrTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<DrTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: DrTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as DrTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof DrTableEntity)[]).forEach((key) => {
