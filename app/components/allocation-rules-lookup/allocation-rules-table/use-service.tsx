...
import { getAllocationRulesTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  z.object({
  rule_key: z.string().min(1, "rule_key is required").max(100),
  rule_name: z.string().min(1, "rule_name is required").max(150),
  rule_type: z.string().min(1, "rule_type is required"),
  rule_value: z.string(),
  scope_type: z.string().min(1, "scope_type is required").max(30),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof AllocationRulesTableEntity>(
      (searchParams.get("sort") as keyof AllocationRulesTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<AllocationRulesTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getAllocationRulesTableEntityApi(module);

 
  // const customResolver: Resolver<AllocationRulesTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<AllocationRulesTableEntity>({
 defaultValues: {
  rule_key: "",
  rule_name: "",
  rule_type: "",
  ...
  effective_to: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"rule_key":"","rule_name":"","rule_type":"","scope_type":"","scope_value":"","priority":0}
  });
  ...
  const onSubmit: SubmitHandler<AllocationRulesTableEntity> = async (values) => {
    // ✅ Validation
  
  if (values.rule_key === undefined || values.rule_key === null) {
    toast.error("rule_key is required");
    return;
  }

  if (values.rule_name === undefined || values.rule_name === null) {
    toast.error("rule_name is required");
    return;
  }

  if (values.rule_type === undefined || values.rule_type === null) {
    toast.error("rule_type is required");
    return;
  }

  if (values.rule_value === undefined || values.rule_value === null) {
    toast.error("rule_value is required");
    return;
  }

  if (values.scope_type === undefined || values.scope_type === null) {
    toast.error("scope_type is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: AllocationRulesTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: AllocationRulesTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as AllocationRulesTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof AllocationRulesTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as AllocationRulesTableEntity);
    setEditId(0);
  };
