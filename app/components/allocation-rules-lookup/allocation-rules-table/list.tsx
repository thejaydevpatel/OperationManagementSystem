...
interface allocation_rules_tablePaginationTableProps {
  rows: AllocationRulesTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<AllocationRulesTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof AllocationRulesTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof AllocationRulesTableEntity>>;
...
const List: React.FC<allocation_rules_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Allocation Rules Table yet`}
        description="Create your first record to get started."
        buttonText={`New Allocation Rules Table`}
       buttonHref="/dashboard/allocation-rules-lookup/allocation-rules-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/allocation-rules-lookup/allocation-rules-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/allocation-rules-lookup/allocation-rules-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
