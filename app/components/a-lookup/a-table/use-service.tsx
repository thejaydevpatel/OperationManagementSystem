...
import { ATableEntity } from "@/app/api/a-lookup/a-table/interface/a-table"
   

import { getATableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof ATableEntity>(
      (searchParams.get("sort") as keyof ATableEntity) || "id"
    );
  ...
  const [list, setList] = useState<ATableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getATableEntityApi(module);

 
  // const customResolver: Resolver<ATableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<ATableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: ATableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as ATableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof ATableEntity)[]).forEach((key) => {
