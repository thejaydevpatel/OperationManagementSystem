...
interface operation_logs_tablePaginationTableProps {
  rows: OperationLogsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<OperationLogsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof OperationLogsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof OperationLogsTableEntity>>;
...
const List: React.FC<operation_logs_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [old_status_id, setOld_status_id] = React.useState<any[]>([]);


const [new_status_id, setNew_status_id] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchNew_status_id = async () => {
    const res = await fetch("/api/status-master-lookup/status-master-table?pageSize=9999");
    const data = await res.json();
    setNew_status_id(data.data || []);
  };
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Operation Logs Table yet`}
        description="Create your first record to get started."
        buttonText={`New Operation Logs Table`}
       buttonHref="/dashboard/operation-logs-lookup/operation-logs-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/operation-logs-lookup/operation-logs-table/new"
                >
...
<TableHead>Old Status Id</TableHead>
<TableHead>New Status Id</TableHead>
...
<TableHead>Notes</TableHead>
  ...
  <TableCell>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {old_status_id.find(o => Number(o.id) === Number(row.old_status_id))?.name || "-"}
      </p>
    </TableCell>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {new_status_id.find(o => Number(o.id) === Number(row.new_status_id))?.name || "-"}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/operation-logs-lookup/operation-logs-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
