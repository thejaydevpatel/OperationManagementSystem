...
import { OperationLogsTableEntity } from "@/app/api/operation-logs-lookup/operation-logs-table/interface/operation-logs-table"
   

import { getOperationLogsTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  ...
  old_status_id: z.number().int().optional(),
  new_status_id: z.number().int().optional(),
  performed_by: z.string().min(1, "performed_by is required"),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof OperationLogsTableEntity>(
      (searchParams.get("sort") as keyof OperationLogsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<OperationLogsTableEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [job_id, setJob_id] = useState<any[]>([]);
 const [old_status_id, setOld_status_id] = useState<any[]>([]);
 const [new_status_id, setNew_status_id] = useState<any[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  old_status_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/status-master-lookup/status-master-table?pageSize=9999`,
  new_status_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/status-master-lookup/status-master-table?pageSize=9999`,
};

  const api = getOperationLogsTableEntityApi(module);

 
  // const customResolver: Resolver<OperationLogsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<OperationLogsTableEntity>({
 defaultValues: {
  action: "",
  performed_by: "",
  notes: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"action":"","performed_by":""}
  });
  ...
  const onSubmit: SubmitHandler<OperationLogsTableEntity> = async (values) => {
    // ✅ Validation
  ...
  if (values.action === undefined || values.action === null) {
    toast.error("action is required");
    return;
  }

  if (values.performed_by === undefined || values.performed_by === null) {
    toast.error("performed_by is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: OperationLogsTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: OperationLogsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as OperationLogsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof OperationLogsTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as OperationLogsTableEntity);
    setEditId(0);
  };
...
if (key === "old_status_id") {
      const endPoint =dropdownEndpoints[key]; 
      if (endPoint) {
      const result = await handleRefreshAPIs(endPoint);
      setOld_status_id(result.data);
    }
    }

if (key === "new_status_id") {
      const endPoint =dropdownEndpoints[key]; 
      if (endPoint) {
      const result = await handleRefreshAPIs(endPoint);
      setNew_status_id(result.data);
    }
    }
  };
