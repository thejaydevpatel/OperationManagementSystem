...
import { LanguageMasterTableEntity } from "@/app/api/language-master-lookup/language-master-table/interface/language-master-table";

interface language_master_tablePaginationTableProps {
  rows: LanguageMasterTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<LanguageMasterTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof LanguageMasterTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof LanguageMasterTableEntity>>;
...
const List: React.FC<language_master_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  ...
  exportToExcel(dataToExport, "Language_Master_Table");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Language Master Table yet`}
        description="Create your first record to get started."
        buttonText={`New Language Master Table`}
       buttonHref="/dashboard/language-master-lookup/language-master-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/language-master-lookup/language-master-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/language-master-lookup/language-master-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
