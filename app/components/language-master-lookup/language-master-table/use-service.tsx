...
import { getLanguageMasterTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof LanguageMasterTableEntity>(
      (searchParams.get("sort") as keyof LanguageMasterTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<LanguageMasterTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getLanguageMasterTableEntityApi(module);

 
  // const customResolver: Resolver<LanguageMasterTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<LanguageMasterTableEntity>({
 defaultValues: {
  name: "",
},  
  ...
  const onSubmit: SubmitHandler<LanguageMasterTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: LanguageMasterTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: LanguageMasterTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as LanguageMasterTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof LanguageMasterTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as LanguageMasterTableEntity);
    setEditId(0);
  };
