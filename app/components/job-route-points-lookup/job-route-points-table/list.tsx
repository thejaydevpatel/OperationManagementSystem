...
interface job_route_points_tablePaginationTableProps {
  rows: JobRoutePointsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<JobRoutePointsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof JobRoutePointsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof JobRoutePointsTableEntity>>;
...
const List: React.FC<job_route_points_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [location_id, setLocation_id] = React.useState<any[]>([]);
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Location":
      location_id.find(o => Number(o.id) === Number(row.location_id))
        ?.name || "-",
    ...
    "Status Name":
      status_id.find(o => Number(o.id) === Number(row.status_id))
        ?.name || "-",
  ...
  exportToExcel(exportData, "Job Route Points");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Job Route Points Table yet`}
        description="Create your first record to get started."
        buttonText={`New Job Route Points Table`}
       buttonHref="/dashboard/job-route-points-lookup/job-route-points-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/job-route-points-lookup/job-route-points-table/new"
                >
...
<TableHead>Notes</TableHead>
  ...
  <TableCell>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {location_id.find(o => Number(o.id) === Number(row.location_id))?.name || "-"}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/job-route-points-lookup/job-route-points-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
