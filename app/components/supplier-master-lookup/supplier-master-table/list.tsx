...
interface supplier_master_tablePaginationTableProps {
  rows: SupplierMasterTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<SupplierMasterTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof SupplierMasterTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof SupplierMasterTableEntity>>;
...
const List: React.FC<supplier_master_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Supplier Master Table yet`}
        description="Create your first record to get started."
        buttonText={`New Supplier Master Table`}
       buttonHref="/dashboard/supplier-master-lookup/supplier-master-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/supplier-master-lookup/supplier-master-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/supplier-master-lookup/supplier-master-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
