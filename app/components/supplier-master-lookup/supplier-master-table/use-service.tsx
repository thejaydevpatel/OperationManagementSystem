...
import { getSupplierMasterTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof SupplierMasterTableEntity>(
      (searchParams.get("sort") as keyof SupplierMasterTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<SupplierMasterTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getSupplierMasterTableEntityApi(module);

 
  // const customResolver: Resolver<SupplierMasterTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: SupplierMasterTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: SupplierMasterTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as SupplierMasterTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof SupplierMasterTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as SupplierMasterTableEntity);
    setEditId(0);
  };
