...
import { PagesEntity } from "@/app/api/master-lookup/pages/interface/pages"
   

import { getPagesEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof PagesEntity>(
      (searchParams.get("sort") as keyof PagesEntity) || "id"
    );
  ...
  const [list, setList] = useState<PagesEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  module_name: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/master-lookup/modules?pageSize=9999`,
};

  const api = getPagesEntityApi(module);

 
  // const customResolver: Resolver<PagesEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  icon: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"page_name":"","page_url":"","icon":""}
  });
  ...
  const onSubmit: SubmitHandler<PagesEntity> = async (values) => {
    // ✅ Validation
  
  if (values.module_name === undefined || values.module_name === null) {
    toast.error("module_name is required");
    return;
  }

  if (values.page_name === undefined || values.page_name === null) {
    toast.error("page_name is required");
    return;
  }

  if (values.page_url === undefined || values.page_url === null) {
    toast.error("page_url is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  response: { data: PagesEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as PagesEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof PagesEntity)[]).forEach((key) => {
  ...
  return {
    state: { register, list, editId, modal, setList,btnLoading, pageLoading, page,
      ...
      setDropDownloading,module_name, },
