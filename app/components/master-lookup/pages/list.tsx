...
interface pagesPaginationTableProps {
  rows: PagesEntity[];
  ...
  setList: React.Dispatch<React.SetStateAction<PagesEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof PagesEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof PagesEntity>>;
...
const List: React.FC<pagesPaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [module_name, setModule_name] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchModule_name = async () => {
    const res = await fetch("/api/master-lookup/modules?pageSize=9999");
    const data = await res.json();
    setModule_name(data.data || []);
  };
 ...
 const handleDragEnd = async (result: DropResult) => {
  ...
  // ✅ Prepare payload
  const payload = updatedRows.map((row, index) => ({
    id: row.id,
    order_index: index + 1,
  }));

  try {
    await fetch("/api/master-lookup/pages/reorder", {
      method: "POST",
      headers: {
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    "Sr No": index + 1,
    "Module Name":
  module_name?.find?.(o => Number(o.id) === Number(row.module_name))?.name || "",
...
const filteredRows = (rows || []).filter((row) => {
  ...
  if ((rows?.length ?? 0) === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Pages yet`}
        description="Create your first record to get started."
        buttonText={`New Pages`}
       buttonHref="/dashboard/master-lookup/pages/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/master-lookup/pages/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/master-lookup/pages/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
