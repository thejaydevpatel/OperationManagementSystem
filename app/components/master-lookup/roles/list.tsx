...
interface rolesPaginationTableProps {
  rows: RolesEntity[];
  ...
  setList: React.Dispatch<React.SetStateAction<RolesEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof RolesEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof RolesEntity>>;
...
const List: React.FC<rolesPaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Roles yet`}
        description="Create your first record to get started."
        buttonText={`New Roles`}
       buttonHref="/dashboard/master-lookup/roles/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/master-lookup/roles/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/master-lookup/roles/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
