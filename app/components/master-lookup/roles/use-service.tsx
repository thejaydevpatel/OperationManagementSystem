...
import { getRolesEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof RolesEntity>(
      (searchParams.get("sort") as keyof RolesEntity) || "id"
    );
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getRolesEntityApi(module);

 
  // const customResolver: Resolver<RolesEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<RolesEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: RolesEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as RolesEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof RolesEntity)[]).forEach((key) => {
