...
import { ModulesEntity } from "@/app/api/master-lookup/modules/interface/modules";

interface modulesPaginationTableProps {
  rows: ModulesEntity[];
  ...
  setList: React.Dispatch<React.SetStateAction<ModulesEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof ModulesEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof ModulesEntity>>;
...
const List: React.FC<modulesPaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
 ...
 if (filteredRows.length === 1 && customPage > 0) {
  ...
  // ✅ Work on full dataset (NOT paginated slice)
  ...
  try {
    await fetch("/api/master-lookup/modules/reorder", {
      method: "POST",
      headers: {
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Modules yet`}
        description="Create your first record to get started."
        buttonText={`New Modules`}
       buttonHref="/dashboard/master-lookup/modules/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/master-lookup/modules/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/master-lookup/modules/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
