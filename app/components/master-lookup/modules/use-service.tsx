...
import { getModulesEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof ModulesEntity>(
      (searchParams.get("sort") as keyof ModulesEntity) || "id"
    );
  ...
  const [list, setList] = useState<ModulesEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getModulesEntityApi(module);

 
  // const customResolver: Resolver<ModulesEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<ModulesEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: ModulesEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as ModulesEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof ModulesEntity)[]).forEach((key) => {
