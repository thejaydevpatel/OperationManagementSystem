...
interface admin_usersPaginationTableProps {
  rows: AdminUsersEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<AdminUsersEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof AdminUsersEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof AdminUsersEntity>>;
...
const List: React.FC<admin_usersPaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
React.useEffect(() => {
  const fetchRole = async () => {
    const res = await fetch("/api/master-lookup/roles?pageSize=9999");
...
"Role":
  role?.find?.(o => Number(o.id) === Number(row.role))?.name || ""
  }));
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Admin Users yet`}
        description="Create your first record to get started."
        buttonText={`New Admin Users`}
       buttonHref="/dashboard/master-lookup/admin-users/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/master-lookup/admin-users/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/master-lookup/admin-users/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
