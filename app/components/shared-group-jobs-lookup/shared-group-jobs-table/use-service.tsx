...
import { SharedGroupJobsTableEntity } from "@/app/api/shared-group-jobs-lookup/shared-group-jobs-table/interface/shared-group-jobs-table"
   

import { getSharedGroupJobsTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  ...
  job_id: z.number().int(),
  pax_assigned: z.number().int(),
  pickup_order_no: z.number().int().optional(),
  drop_order_no: z.number().int().optional(),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof SharedGroupJobsTableEntity>(
      (searchParams.get("sort") as keyof SharedGroupJobsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<SharedGroupJobsTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getSharedGroupJobsTableEntityApi(module);

 
  // const customResolver: Resolver<SharedGroupJobsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<SharedGroupJobsTableEntity>({
 defaultValues: {
  pax_assigned: 0,
  pickup_order_no: 0,
  drop_order_no: 0,
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"pax_assigned":0,"pickup_order_no":0,"drop_order_no":0}
  });
  ...
  const onSubmit: SubmitHandler<SharedGroupJobsTableEntity> = async (values) => {
    // ✅ Validation
  ...
  if (values.pax_assigned === undefined || values.pax_assigned === null) {
    toast.error("pax_assigned is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: SharedGroupJobsTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: SharedGroupJobsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as SharedGroupJobsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof SharedGroupJobsTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as SharedGroupJobsTableEntity);
    setEditId(0);
  };
