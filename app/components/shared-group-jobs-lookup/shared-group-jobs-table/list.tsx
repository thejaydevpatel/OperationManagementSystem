...
interface shared_group_jobs_tablePaginationTableProps {
  rows: SharedGroupJobsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<SharedGroupJobsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof SharedGroupJobsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof SharedGroupJobsTableEntity>>;
...
const List: React.FC<shared_group_jobs_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [group_id, setGroup_id] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchGroup_id = async () => {
    const res = await fetch("/api/shared-groupings-lookup/shared-groupings-table?pageSize=9999");
    const data = await res.json();
    setGroup_id(data.data || []);
  };
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Shared Group Jobs Table yet`}
        description="Create your first record to get started."
        buttonText={`New Shared Group Jobs Table`}
       buttonHref="/dashboard/shared-group-jobs-lookup/shared-group-jobs-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/shared-group-jobs-lookup/shared-group-jobs-table/new"
                >
                  <Button className="min-w-[150px]">
                    <Plus className="mr-2 h-4 w-4" />
                    Add New Shared Group Job
                  </Button>
                </Link>
...
<TableHead>Pax Assigned</TableHead>
<TableHead>Pickup Order No</TableHead>
<TableHead>Drop Order No</TableHead>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/shared-group-jobs-lookup/shared-group-jobs-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
