...
interface placard_details_tablePaginationTableProps {
  rows: PlacardDetailsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<PlacardDetailsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof PlacardDetailsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof PlacardDetailsTableEntity>>;
...
const List: React.FC<placard_details_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  ...
  exportToExcel(exportData, "Placard_Details");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Placard Details Table yet`}
        description="Create your first record to get started."
        buttonText={`New Placard Details Table`}
       buttonHref="/dashboard/placard-details-lookup/placard-details-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/placard-details-lookup/placard-details-table/new"
                >
                  <Button className="min-w-[150px]">
                    <Plus className="mr-2 h-4 w-4" />
                    Add New Placard Detail
                  </Button>
                </Link>
...
<TableHead>Placard Text</TableHead>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/placard-details-lookup/placard-details-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
