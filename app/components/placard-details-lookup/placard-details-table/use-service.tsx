...
import { getPlacardDetailsTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  z.object({
  job_id: z.number().int(),
  guest_name: z.string().min(1, "guest_name is required").max(150),
  placard_text: z.string().min(1, "placard_text is required").max(200),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof PlacardDetailsTableEntity>(
      (searchParams.get("sort") as keyof PlacardDetailsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<PlacardDetailsTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getPlacardDetailsTableEntityApi(module);

 
  // const customResolver: Resolver<PlacardDetailsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<PlacardDetailsTableEntity>({
 defaultValues: {
  guest_name: "",
  placard_text: "",
  language: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"guest_name":"","placard_text":"","language":""}
  });
  ...
  const onSubmit: SubmitHandler<PlacardDetailsTableEntity> = async (values) => {
    // ✅ Validation
  ...
  if (values.guest_name === undefined || values.guest_name === null) {
    toast.error("guest_name is required");
    return;
  }

  if (values.placard_text === undefined || values.placard_text === null) {
    toast.error("placard_text is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: PlacardDetailsTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: PlacardDetailsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as PlacardDetailsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof PlacardDetailsTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as PlacardDetailsTableEntity);
    setEditId(0);
  };
