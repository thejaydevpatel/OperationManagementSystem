...
import { getLocationMasterTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  ...
  zone_type: z.string().min(1, "zone_type is required"),
  parent_zone_id: z.number().int().optional(),
  });
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof LocationMasterTableEntity>(
      (searchParams.get("sort") as keyof LocationMasterTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<LocationMasterTableEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [parent_zone_id, setParent_zone_id] = useState<any[]>([]);


const dropdownEndpoints: Record<string, string | null> = {
  parent_zone_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/location-master-lookup/location-master-table?pageSize=9999`,
};

  const api = getLocationMasterTableEntityApi(module);

 
  // const customResolver: Resolver<LocationMasterTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<LocationMasterTableEntity>({
 defaultValues: {
  name: "",
  latitude: "",
  longitude: "",
  zone_type: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"name":"","latitude":0,"longitude":0,"zone_type":""}
  });
  ...
  const onSubmit: SubmitHandler<LocationMasterTableEntity> = async (values) => {
    // ✅ Validation
  ...
  if (values.zone_type === undefined || values.zone_type === null) {
    toast.error("zone_type is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: LocationMasterTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: LocationMasterTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as LocationMasterTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof LocationMasterTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as LocationMasterTableEntity);
    setEditId(0);
  };
  ...
  return {
    state: { register, list, editId, modal, setList,btnLoading, pageLoading, page,
      ...
      setDropDownloading,parent_zone_id, },
