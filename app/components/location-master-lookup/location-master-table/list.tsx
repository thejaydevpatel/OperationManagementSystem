...
interface location_master_tablePaginationTableProps {
  rows: LocationMasterTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<LocationMasterTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof LocationMasterTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof LocationMasterTableEntity>>;
...
const List: React.FC<location_master_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [parent_zone_id, setParent_zone_id] = React.useState<any[]>([]);
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Location Master Table yet`}
        description="Create your first record to get started."
        buttonText={`New Location Master Table`}
       buttonHref="/dashboard/location-master-lookup/location-master-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/location-master-lookup/location-master-table/new"
                >
...
<TableHead>Parent Zone Id</TableHead>
  ...
  <TableCell>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {parent_zone_id.find(o => Number(o.id) === Number(row.parent_zone_id))?.name || ""}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/location-master-lookup/location-master-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
