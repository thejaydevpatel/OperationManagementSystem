...
import { UsersTableEntity } from "@/app/api/users-lookup/users-table/interface/users-table"
   

import { getUsersTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof UsersTableEntity>(
      (searchParams.get("sort") as keyof UsersTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<UsersTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getUsersTableEntityApi(module);

 
  // const customResolver: Resolver<UsersTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<UsersTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: UsersTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as UsersTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof UsersTableEntity)[]).forEach((key) => {
