...
interface users_tablePaginationTableProps {
  rows: UsersTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<UsersTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof UsersTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof UsersTableEntity>>;
...
const List: React.FC<users_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  try {
    ...
    exportToExcel(exportData, "Users_List");
  } catch (error) {
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Users Table yet`}
        description="Create your first record to get started."
        buttonText={`New Users Table`}
       buttonHref="/dashboard/users-lookup/users-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/users-lookup/users-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/users-lookup/users-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
