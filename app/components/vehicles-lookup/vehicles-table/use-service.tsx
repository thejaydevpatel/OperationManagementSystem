...
import { VehiclesTableEntity } from "@/app/api/vehicles-lookup/vehicles-table/interface/vehicles-table"
   

import { getVehiclesTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof VehiclesTableEntity>(
      (searchParams.get("sort") as keyof VehiclesTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<VehiclesTableEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [vehicle_type_id, setVehicle_type_id] = useState<any[]>([]);
 const [supplier_id, setSupplier_id] = useState<any[]>([]);
 const [vehicle_status_id, setVehicle_status_id] = useState<any[]>([]);


const dropdownEndpoints: Record<string, string | null> = {
  vehicle_type_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/vehicle-types-lookup/vehicle-types-table?pageSize=9999`,
  supplier_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/supplier-master-lookup/supplier-master-table?pageSize=9999`,
  vehicle_status_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/status-master-lookup/status-master-table?pageSize=9999`,
};

  const api = getVehiclesTableEntityApi(module);

 
  // const customResolver: Resolver<VehiclesTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  if (values.registration_number === undefined || values.registration_number === null) {
    toast.error("registration_number is required");
    return;
  }

  if (values.owner_type === undefined || values.owner_type === null) {
    toast.error("owner_type is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  response: { data: VehiclesTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as VehiclesTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof VehiclesTableEntity)[]).forEach((key) => {
  ...
  const handleDropDown = async (key: string) => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 500));
      ...
      setVehicle_type_id(result.data);
    }
    }
...
if (key === "vehicle_status_id") {
      const endPoint =dropdownEndpoints[key]; 
      if (endPoint) {
      const result = await handleRefreshAPIs(endPoint);
      setVehicle_status_id(result.data);
    }
    }
  };
  ...
  return {
    state: { register, list, editId, modal, setList,btnLoading, pageLoading, page,
      ...
      setDropDownloading,vehicle_type_id,
 supplier_id,
 vehicle_status_id, },
