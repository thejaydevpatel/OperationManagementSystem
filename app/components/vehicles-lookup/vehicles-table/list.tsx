...
interface vehicles_tablePaginationTableProps {
  rows: VehiclesTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<VehiclesTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof VehiclesTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof VehiclesTableEntity>>;
...
const List: React.FC<vehicles_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [vehicle_type_id, setVehicle_type_id] = React.useState<any[]>([]);


const [supplier_id, setSupplier_id] = React.useState<any[]>([]);


const [vehicle_status_id, setVehicle_status_id] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchVehicle_status_id = async () => {
    const res = await fetch("/api/status-master-lookup/status-master-table?pageSize=9999");
    const data = await res.json();
    setVehicle_status_id(data.data || []);
  };

  fetchVehicle_status_id();
}, []);
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Vehicle Status":
      vehicle_status_id.find(o => Number(o.id) === Number(row.vehicle_status_id))?.name || "-",
  ...
  exportToExcel(exportData, "Vehicles_Table");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Vehicles Table yet`}
        description="Create your first record to get started."
        buttonText={`New Vehicles Table`}
       buttonHref="/dashboard/vehicles-lookup/vehicles-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/vehicles-lookup/vehicles-table/new"
                >
...
<TableHead>Vehicle Status Id</TableHead>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/vehicles-lookup/vehicles-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
