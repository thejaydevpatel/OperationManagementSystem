...
import { FTableEntity } from "@/app/api/f-lookup/f-table/interface/f-table"
   

import { getFTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof FTableEntity>(
      (searchParams.get("sort") as keyof FTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<FTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getFTableEntityApi(module);

 
  // const customResolver: Resolver<FTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<FTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: FTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as FTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof FTableEntity)[]).forEach((key) => {
