...
interface guide_price_tablePaginationTableProps {
  rows: GuidePriceTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<GuidePriceTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof GuidePriceTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof GuidePriceTableEntity>>;
...
const List: React.FC<guide_price_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Guide Price Table yet`}
        description="Create your first record to get started."
        buttonText={`New Guide Price Table`}
       buttonHref="/dashboard/guide-price-lookup/guide-price-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/guide-price-lookup/guide-price-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/guide-price-lookup/guide-price-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
