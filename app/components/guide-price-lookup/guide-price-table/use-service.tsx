...
import { getGuidePriceTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof GuidePriceTableEntity>(
      (searchParams.get("sort") as keyof GuidePriceTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<GuidePriceTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getGuidePriceTableEntityApi(module);

 
  // const customResolver: Resolver<GuidePriceTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<GuidePriceTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: GuidePriceTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as GuidePriceTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof GuidePriceTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as GuidePriceTableEntity);
    setEditId(0);
  };
