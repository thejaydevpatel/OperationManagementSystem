...
import { VehicleTypesTableEntity } from "@/app/api/vehicle-types-lookup/vehicle-types-table/interface/vehicle-types-table"
   

import { getVehicleTypesTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof VehicleTypesTableEntity>(
      (searchParams.get("sort") as keyof VehicleTypesTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<VehicleTypesTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getVehicleTypesTableEntityApi(module);

 
  // const customResolver: Resolver<VehicleTypesTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<VehicleTypesTableEntity>({
 defaultValues: {
  name: "",
  capacity: 0,
  luggage_capacity: 0,
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"name":"","capacity":0,"luggage_capacity":0}
  });
  ...
  const onSubmit: SubmitHandler<VehicleTypesTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: VehicleTypesTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: VehicleTypesTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as VehicleTypesTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof VehicleTypesTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as VehicleTypesTableEntity);
    setEditId(0);
  };
