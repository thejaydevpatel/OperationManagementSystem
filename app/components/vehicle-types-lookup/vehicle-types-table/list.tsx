...
interface vehicle_types_tablePaginationTableProps {
  rows: VehicleTypesTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<VehicleTypesTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof VehicleTypesTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof VehicleTypesTableEntity>>;
...
const List: React.FC<vehicle_types_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Vehicle Types Table yet`}
        description="Create your first record to get started."
        buttonText={`New Vehicle Types Table`}
       buttonHref="/dashboard/vehicle-types-lookup/vehicle-types-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/vehicle-types-lookup/vehicle-types-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/vehicle-types-lookup/vehicle-types-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
