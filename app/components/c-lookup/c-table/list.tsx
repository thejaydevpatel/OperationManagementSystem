...
interface c_tablePaginationTableProps {
  rows: CTableEntity[];
  ...
  setList: React.Dispatch<React.SetStateAction<CTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof CTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof CTableEntity>>;
...
const List: React.FC<c_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any C Table yet`}
        description="Create your first record to get started."
        buttonText={`New C Table`}
       buttonHref="/dashboard/c-lookup/c-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/c-lookup/c-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/c-lookup/c-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
