...
import { CTableEntity } from "@/app/api/c-lookup/c-table/interface/c-table"
   

import { getCTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof CTableEntity>(
      (searchParams.get("sort") as keyof CTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<CTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getCTableEntityApi(module);

 
  // const customResolver: Resolver<CTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<CTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  response: { data: CTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as CTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof CTableEntity)[]).forEach((key) => {
