...
import { DriverDutyTableEntity } from "@/app/api/driver-duty-lookup/driver-duty-table/interface/driver-duty-table"
   

import { getDriverDutyTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  ...
  duty_start_time: z.string(),
  duty_end_time: z.string().optional(),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof DriverDutyTableEntity>(
      (searchParams.get("sort") as keyof DriverDutyTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<DriverDutyTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getDriverDutyTableEntityApi(module);

 
  // const customResolver: Resolver<DriverDutyTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  const onSubmit: SubmitHandler<DriverDutyTableEntity> = async (values) => {
    // ✅ Validation
  ...
  if (values.duty_start_time === undefined || values.duty_start_time === null) {
    toast.error("duty_start_time is required");
    return;
  }

  if (values.status_id === undefined || values.status_id === null) {
    toast.error("status_id is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  response: { data: DriverDutyTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as DriverDutyTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof DriverDutyTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as DriverDutyTableEntity);
    setEditId(0);
  };
