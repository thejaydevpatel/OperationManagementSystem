...
interface driver_allocation_tablePaginationTableProps {
  rows: DriverAllocationTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<DriverAllocationTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof DriverAllocationTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof DriverAllocationTableEntity>>;
...
const List: React.FC<driver_allocation_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Driver Allocation Table yet`}
        description="Create your first record to get started."
        buttonText={`New Driver Allocation Table`}
       buttonHref="/dashboard/driver-allocation-lookup/driver-allocation-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/driver-allocation-lookup/driver-allocation-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/driver-allocation-lookup/driver-allocation-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
