...
interface shared_group_route_points_tablePaginationTableProps {
  rows: SharedGroupRoutePointsTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<SharedGroupRoutePointsTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof SharedGroupRoutePointsTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof SharedGroupRoutePointsTableEntity>>;
...
const List: React.FC<shared_group_route_points_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [locarion_id, setLocarion_id] = React.useState<any[]>([]);
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Shared Group Route Points Table yet`}
        description="Create your first record to get started."
        buttonText={`New Shared Group Route Points Table`}
       buttonHref="/dashboard/shared-group-route-points-lookup/shared-group-route-points-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/shared-group-route-points-lookup/shared-group-route-points-table/new"
                >
                  <Button className="min-w-[150px]">
                    <Plus className="mr-2 h-4 w-4" />
                    Add New Shared Group Route
                  </Button>
                </Link>
...
<TableHead>Locarion Id</TableHead>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/shared-group-route-points-lookup/shared-group-route-points-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
