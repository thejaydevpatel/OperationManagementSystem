...
import { getSharedGroupRoutePointsTableEntityApi } from "./service";
...
const getSchema = (isEdit: boolean) =>
  ...
  locarion_id: z.number().int(),
  stop_type: z.string().min(1, "stop_type is required"),
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof SharedGroupRoutePointsTableEntity>(
      (searchParams.get("sort") as keyof SharedGroupRoutePointsTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<SharedGroupRoutePointsTableEntity[]>([]);
  const [editId, setEditId] = useState<number>(0);
  const [modal, setModal] = useState<boolean>(false);
  
 const [group_id, setGroup_id] = useState<any[]>([]);
 const [locarion_id, setLocarion_id] = useState<any[]>([]);
 const [status_id, setStatus_id] = useState<any[]>([]);


const dropdownEndpoints: Record<string, string | null> = {
  group_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/shared-groupings-lookup/shared-groupings-table?pageSize=9999`,
  locarion_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/location-master-lookup/location-master-table?pageSize=9999`,
  status_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/status-master-lookup/status-master-table?pageSize=9999`,
};

  const api = getSharedGroupRoutePointsTableEntityApi(module);

 
  // const customResolver: Resolver<SharedGroupRoutePointsTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<SharedGroupRoutePointsTableEntity>({
 defaultValues: {
  order_no: 0,
  stop_type: "",
  eta: "",
  ...
  const onSubmit: SubmitHandler<SharedGroupRoutePointsTableEntity> = async (values) => {
    // ✅ Validation
  
  if (values.group_id === undefined || values.group_id === null) {
    toast.error("group_id is required");
    return;
  }
  ...
  if (values.locarion_id === undefined || values.locarion_id === null) {
    toast.error("locarion_id is required");
    return;
  }
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: SharedGroupRoutePointsTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: SharedGroupRoutePointsTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as SharedGroupRoutePointsTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof SharedGroupRoutePointsTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as SharedGroupRoutePointsTableEntity);
    setEditId(0);
  };
