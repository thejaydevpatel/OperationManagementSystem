...
interface admin_users_tablePaginationTableProps {
  rows: AdminUsersTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<AdminUsersTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof AdminUsersTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof AdminUsersTableEntity>>;
...
const List: React.FC<admin_users_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  ...
  exportToExcel(exportData, "Admin_Users_Table", "Admin Users");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Admin Users Table yet`}
        description="Create your first record to get started."
        buttonText={`New Admin Users Table`}
       buttonHref="/dashboard/admin-users-lookup/admin-users-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/admin-users-lookup/admin-users-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/admin-users-lookup/admin-users-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
