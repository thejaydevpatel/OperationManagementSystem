...
import { AdminUsersTableEntity } from "@/app/api/admin-users-lookup/admin-users-table/interface/admin-users-table"
   

import { getAdminUsersTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof AdminUsersTableEntity>(
      (searchParams.get("sort") as keyof AdminUsersTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<AdminUsersTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getAdminUsersTableEntityApi(module);

 
  // const customResolver: Resolver<AdminUsersTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
 ...
 defaultValues: {
  ...
  // user_code: "",
    ...
    // resolver: zodResolver(getSchema(false)),
      // defaultValues:{"name":"","email":"","password":"","role":"","user_code":""}
  });
  ...
  const onSubmit: SubmitHandler<AdminUsersTableEntity> = async (values) => {
    // ✅ Validation
  ...
  // if (values.user_code === undefined || values.user_code === null) {
  //   toast.error("user_code is required");
  //   return;
  // }
...
await nevigateListOrEdit(true);
  ...
  response: { data: AdminUsersTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as AdminUsersTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof AdminUsersTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as AdminUsersTableEntity);
    setEditId(0);
  };
