...
import { getGuideAllocationTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof GuideAllocationTableEntity>(
      (searchParams.get("sort") as keyof GuideAllocationTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<GuideAllocationTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getGuideAllocationTableEntityApi(module);

 
  // const customResolver: Resolver<GuideAllocationTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<GuideAllocationTableEntity>({
 defaultValues: {
  report_time: "",
  actual_start_time: "",
  actual_end_time: "",
  ...
  const onSubmit: SubmitHandler<GuideAllocationTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: GuideAllocationTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: GuideAllocationTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as GuideAllocationTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof GuideAllocationTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as GuideAllocationTableEntity);
    setEditId(0);
  };
