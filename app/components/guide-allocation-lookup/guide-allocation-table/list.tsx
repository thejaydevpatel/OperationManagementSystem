...
import { GuideAllocationTableEntity } from "@/app/api/guide-allocation-lookup/guide-allocation-table/interface/guide-allocation-table";

interface guide_allocation_tablePaginationTableProps {
  rows: GuideAllocationTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<GuideAllocationTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof GuideAllocationTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof GuideAllocationTableEntity>>;
...
const List: React.FC<guide_allocation_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
React.useEffect(() => {
  const fetchGuide_id = async () => {
    const res = await fetch("/api/tour-guides-lookup/tour-guides-table?pageSize=9999");
    const data = await res.json();
    setGuide_id(data.data || []);
  };
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Supplier":
      supplier_id.find(o => Number(o.id) === Number(row.supplier_id))
        ?.name || "-",

    "Guide":
      guide_id.find(o => Number(o.id) === Number(row.guide_id))
        ?.name || "-",

    "Allocation Status":
      allocation_status_id.find(o => Number(o.id) === Number(row.allocation_status_id))
        ?.name || "-",

    "Report Time": row.report_time
      ? new Date(row.report_time).toLocaleString("en-GB")
      : "-",

    "Actual Start Time": row.actual_start_time
      ? new Date(row.actual_start_time).toLocaleString("en-GB")
      : "-",

    "Actual End Time": row.actual_end_time
      ? new Date(row.actual_end_time).toLocaleString("en-GB")
      : "-",

    "Extra Charge": row.extra_charge || "-",

    "Status": row.is_active ? "Active" : "Suspended",
  }));

  exportToExcel(exportData, "Guide Allocation");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Guide Allocation Table yet`}
        description="Create your first record to get started."
        buttonText={`New Guide Allocation Table`}
       buttonHref="/dashboard/guide-allocation-lookup/guide-allocation-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/guide-allocation-lookup/guide-allocation-table/new"
                >
...
<TableHead>Extra Charge</TableHead>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {guide_id.find(o => Number(o.id) === Number(row.guide_id))?.name || "-"}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/guide-allocation-lookup/guide-allocation-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
