...
interface driver_availability_tablePaginationTableProps {
  rows: DriverAvailabilityTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number, field: string) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<DriverAvailabilityTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof DriverAvailabilityTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof DriverAvailabilityTableEntity>>;
...
const List: React.FC<driver_availability_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const [driver_id, setDriver_id] = React.useState<any[]>([]);


const [current_location_id, setCurrent_location_id] = React.useState<any[]>([]);


const [current_job_id, setCurrent_job_id] = React.useState<any[]>([]);
...
React.useEffect(() => {
  const fetchDriver_id = async () => {
    const res = await fetch("/api/drivers-lookup/drivers-table?pageSize=9999");
    const data = await res.json();
    setDriver_id(data.data || []);
  };
...
React.useEffect(() => {
  const fetchCurrent_location_id = async () => {
    const res = await fetch("/api/location-master-lookup/location-master-table?pageSize=9999");
    const data = await res.json();
    setCurrent_location_id(data.data || []);
  };

  fetchCurrent_location_id();
}, []);


React.useEffect(() => {
  const fetchCurrent_job_id = async () => {
    const res = await fetch("/api/operation-jobs-lookup/operation-jobs-table?pageSize=9999");
    const data = await res.json();
    setCurrent_job_id(data.data || []);
  };
...
const handleExport = () => {
  const exportData = filteredRows.map((row) => ({
    ...
    Location: current_location_id.find(o => Number(o.id) === Number(row.current_location_id))?.name || "-",
    Job: current_job_id.find(o => Number(o.id) === Number(row.current_job_id))?.external_booking_id || "-",
    NextFreeTime: row.next_free_time
      ? new Date(row.next_free_time).toLocaleString("en-GB")
      : "-",
    Status: row.is_active ? "Active" : "Suspended",
  }));

  exportToExcel(exportData, "Driver_Availability");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Driver Availability Table yet`}
        description="Create your first record to get started."
        buttonText={`New Driver Availability Table`}
       buttonHref="/dashboard/driver-availability-lookup/driver-availability-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/driver-availability-lookup/driver-availability-table/new"
                >
...
<TableHead>Current Location Id</TableHead>
<TableHead>Current Job Id</TableHead>
<TableHead>Next Free Time</TableHead>
  ...
  <div className="flex items-center gap-2">
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {current_location_id.find(o => Number(o.id) === Number(row.current_location_id))?.name || "-"}
      </p>
    </TableCell>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {current_job_id.find(o => Number(o.id) === Number(row.current_job_id))?.external_booking_id || "-"}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/driver-availability-lookup/driver-availability-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
