...
import { getTourGuidesTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof TourGuidesTableEntity>(
      (searchParams.get("sort") as keyof TourGuidesTableEntity) || "id"
    );
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  guide_status_id: `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/status-master-lookup/status-master-table?pageSize=9999`,
};
...
await nevigateListOrEdit(true);
  ...
  response: { data: TourGuidesTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as TourGuidesTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof TourGuidesTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as TourGuidesTableEntity);
    setEditId(0);
  };
...
if (key === "guide_status_id") {
      const endPoint =dropdownEndpoints[key]; 
      if (endPoint) {
      const result = await handleRefreshAPIs(endPoint);
      setGuide_status_id(result.data);
    }
    }
  };
