...
interface tour_guides_tablePaginationTableProps {
  rows: TourGuidesTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<TourGuidesTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof TourGuidesTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof TourGuidesTableEntity>>;
...
const List: React.FC<tour_guides_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
React.useEffect(() => {
  const fetchGuide_status_id = async () => {
    const res = await fetch("/api/status-master-lookup/status-master-table?pageSize=9999");
    const data = await res.json();
    setGuide_status_id(data.data || []);
  };

  fetchGuide_status_id();
}, []);
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Tour Guides Table yet`}
        description="Create your first record to get started."
        buttonText={`New Tour Guides Table`}
       buttonHref="/dashboard/tour-guides-lookup/tour-guides-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/tour-guides-lookup/tour-guides-table/new"
                >
...
<TableHead>Notes</TableHead>
  ...
  <TableCell>
    ...
    <TableCell>
      <p className="text-muted-foreground text-sm font-normal">
    {guide_status_id.find(o => Number(o.id) === Number(row.guide_status_id))?.name || "-"}
      </p>
    </TableCell>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/tour-guides-lookup/tour-guides-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
