...
interface guest_review_tablePaginationTableProps {
  rows: GuestReviewTableEntity[];
  onDelete: (Id: number) => void;
 onStatus: (Id: number, field: string) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<GuestReviewTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof GuestReviewTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof GuestReviewTableEntity>>;
...
const List: React.FC<guest_review_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  const exportData = filteredRows.map((row, index) => ({
    ...
    "Review Target Type": row.review_target_type || "-",
    "Review Target Id": row.review_target_id || "-",
    "Rating": row.rating || "-",
    "Review Text": row.review_text || "-",
    "Guest Name": row.guest_name || "-",
    "Guest Email": row.guest_email || "-",
    "Is Anonymous": row.is_anonymous ? "Yes" : "No",
    "Is Verified": row.is_verified ? "Yes" : "No",
    "Status": row.is_active ? "Active" : "Suspended",
  }));

  exportToExcel(exportData, "Guest_Review_Table");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Guest Review Table yet`}
        description="Create your first record to get started."
        buttonText={`New Guest Review Table`}
       buttonHref="/dashboard/guest-review-lookup/guest-review-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/guest-review-lookup/guest-review-table/new"
                >
...
<TableHead>Review Target Type</TableHead>
<TableHead>Review Target Id</TableHead>
...
<TableHead>Is Verified</TableHead>
 ...
 <TableCell>
                              <div className="flex items-center gap-2">
                                <Switch
  checked={row.is_anonymous}
  onCheckedChange={() =>
    onStatus(row.id, "is_anonymous")
  }
/>
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/guest-review-lookup/guest-review-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
