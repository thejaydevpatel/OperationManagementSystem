...
interface supplier_types_tablePaginationTableProps {
  rows: SupplierTypesTableEntity[];
  onDelete: (Id: number) => void;
  onStatus: (Id: number) => void;
  onView: (Id: number) => void;
  onListToggle: () => void;
  module: ModuleDetailsString;
  setList: React.Dispatch<React.SetStateAction<SupplierTypesTableEntity[]>>;
  customPage: number;
  customRowsPerPage: number;
  customSetPage: React.Dispatch<React.SetStateAction<number>>;
  customTotalRecordObj: Pagination;
  customSortBy: keyof SupplierTypesTableEntity;
  customSetSortBy: React.Dispatch<React.SetStateAction<keyof SupplierTypesTableEntity>>;
...
const List: React.FC<supplier_types_tablePaginationTableProps> = ({
  rows,
  onDelete,
  onStatus,
  onView,
...
const handleExport = () => {
  ...
  exportToExcel(dataToExport, "Supplier_Types_Table");
};
...
const filteredRows = rows.filter((row) => {
  ...
  if (rows.length === 0 && !pageLoading) {
    return (
      <EmptyState
        title={`You haven't created any Supplier Types Table yet`}
        description="Create your first record to get started."
        buttonText={`New Supplier Types Table`}
       buttonHref="/dashboard/supplier-types-lookup/supplier-types-table/new"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
                <Link
                  href="/dashboard/supplier-types-lookup/supplier-types-table/new"
                >
                            ...
                            <TableCell>
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                        onClick={() => router.push(`/dashboard/supplier-types-lookup/supplier-types-table/${row.id}`)}                                  >
                                    Edit
                                  </DropdownMenuItem>
