...
import { getSupplierTypesTableEntityApi } from "./service";
...
const router = useRouter();
  ...
  const [pageLoading, setPageLoading] = useState(true); 
   const [dropDownloading, setDropDownloading] = useState<{
    ...
    const [sortBy, setSortBy] = useState<keyof SupplierTypesTableEntity>(
      (searchParams.get("sort") as keyof SupplierTypesTableEntity) || "id"
    );
  ...
  const [list, setList] = useState<SupplierTypesTableEntity[]>([]);
...
const dropdownEndpoints: Record<string, string | null> = {
  ...
  const api = getSupplierTypesTableEntityApi(module);

 
  // const customResolver: Resolver<SupplierTypesTableEntity> = async (
  //     values,
  //     _context,
  //     options
  //   ) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<SupplierTypesTableEntity>({
 defaultValues: {
  name: "",
},  
  ...
  const onSubmit: SubmitHandler<SupplierTypesTableEntity> = async (values) => {
    // ✅ Validation
...
await nevigateListOrEdit(true);
  ...
  const handleImageUploads = async (
      values: SupplierTypesTableEntity,
      id: number
    ): Promise<void> => {
  ...
  response: { data: SupplierTypesTableEntity[]; message: string },
  Id: number
): Promise<void> => { 
    if (response.data.length > 0) {
      const record = response.data[0] as SupplierTypesTableEntity;
      const remapped: Record<string, unknown> = { ...record };
      (Object.keys(record) as (keyof SupplierTypesTableEntity)[]).forEach((key) => {
  ...
  const handleClear = (): void => {
    reset({} as SupplierTypesTableEntity);
    setEditId(0);
  };
